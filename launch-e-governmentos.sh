...
#
# Usage:
#   ./launch-e-governmentos.sh              # Start everything
#   ./launch-e-governmentos.sh --help       # Show help
#
################################################################################
...
AGENT_DIR="$SCRIPT_DIR/packages/e-governmentos-agent/dist"
BROWSER_APP="/Users/Mukira/chromium/src/out/Release/e-GovernmentOS.app"
BROWSER_BINARY="$BROWSER_APP/Contents/MacOS/e-GovernmentOS"

# Server ports
SERVER_LOG="$SCRIPT_DIR/e-governmentos-server.log"
SERVER_PID_FILE="$SCRIPT_DIR/e-governmentos-server.pid"
...
USAGE:
    ./launch-e-governmentos.sh [OPTIONS]
...
check_dependencies() {
    ...
    if [[ ! -d "$SERVER_DIR" ]]; then
        ...
        log_error "Run: git clone https://github.com/e-governmentos-ai/e-GovernmentOS-server.git"
        exit 1
    fi
    ...
    if [[ ! -d "$AGENT_DIR" ]]; then
        ...
        cd "$SCRIPT_DIR/packages/e-governmentos-agent"
