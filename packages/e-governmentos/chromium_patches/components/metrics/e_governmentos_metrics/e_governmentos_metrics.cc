diff --git a/components/metrics/e_governmentos_metrics/e_governmentos_metrics.cc b/components/metrics/e_governmentos_metrics/e_governmentos_metrics.cc
new file mode 100644
index 0000000000000..02a1dc121bb7a
--- /dev/null
+++ b/components/metrics/e_governmentos_metrics/e_governmentos_metrics.cc
@@ -0,0 +1,100 @@
...
+e-GovernmentOSMetricsService* GetMetricsService() {
...
+  return e-GovernmentOSMetricsServiceFactory::GetForBrowserContext(profile);
+}
+
...
+  } else {
+    VLOG(1) << "e_governmentos: Metrics service not available for event: " << event_name;
+  }
+}
+
+}  // namespace
+
+// static
+void e-GovernmentOSMetrics::Log(const std::string& event_name, double sample_rate) {
+  Log(event_name, base::Value::Dict(), sample_rate);
+}
+
+// static
+void e-GovernmentOSMetrics::Log(const std::string& event_name,
...
+
+// static
+void e-GovernmentOSMetrics::Log(const std::string& event_name, base::Value::Dict properties,
+                           double sample_rate) {
