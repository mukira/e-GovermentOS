...
+  }
+
+  // e-GovernmentOS Agent - toggles contextual side panel on active tab.
...
+                  LOG(WARNING) << "e_governmentos: No active tab for Agent action";
+                  return;
+                }
+
...
+                        ->enabled_extensions()
+                        .GetByID(extensions::e_governmentos::kAgentV2ExtensionId);
+                if (!extension) {
+                  LOG(WARNING) << "e_governmentos: Agent extension not found";
+                  return;
+                }
+
+                int tab_id = extensions::ExtensionTabUtil::GetTabId(contents);
+                LOG(INFO) << "e_governmentos: Agent toolbar action for tab_id="
+                          << tab_id;
+
+                extensions::SidePanelService* service =
+                    extensions::SidePanelService::Get(profile);
+                if (!service) {
+                  LOG(WARNING) << "e_governmentos: SidePanelService not found";
+                  return;
+                }
+
...
+
+                if (!result.has_value()) {
+                  LOG(WARNING) << "e_governmentos: Agent toggle failed: "
+                               << result.error();
+                } else {
+                  LOG(INFO) << "e_governmentos: Agent toggle result: "
+                            << result.value();
+                }
+              },
+              bwi))
+          .SetActionId(kActione-GovernmentOSAgent)
+          .SetText(u"Assistant")
+          .SetTooltipText(u"Ask e-GovernmentOS")
