...
+
+// Preference names
+const char kClashOfGptsPaneProvidersPref[] = "e_governmentos.clash_of_gpts.pane_providers";  // Per-pane selections
+const char kClashOfGptsLastUrlsPref[] = "e_governmentos.clash_of_gpts.last_urls";
+const char kClashOfGptsPaneCountPref[] = "e_governmentos.clash_of_gpts.pane_count";
+
...
+    widget_->Show();
+    widget_->Activate();
+    e_governmentos_metrics::e-GovernmentOSMetrics::Log("llmhub.shown");
+  }
+}
+
...
+    LOG(ERROR) << "[e_governmentos] Invalid provider index: " << provider_index;
+    return;
+  }
+
...
+  SaveState();
+  
+  e_governmentos_metrics::e-GovernmentOSMetrics::Log("llmhub.panecount.changed", 
