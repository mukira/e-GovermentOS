...
+        LOG(INFO) << "e_governmentos: Toolbar action clicked for extension="
+                  << extension_id;
+
+        // Get the active tab.
...
+        if (!active_contents) {
+          LOG(WARNING) << "e_governmentos: No active tab contents";
+          return;
+        }
+
+        int tab_id = extensions::ExtensionTabUtil::GetTabId(active_contents);
+        LOG(INFO) << "e_governmentos: Active tab_id=" << tab_id;
+
...
+
+        if (!extension) {
+          LOG(WARNING) << "e_governmentos: Extension not found: " << extension_id;
+          return;
+        }
+
...
+
+        if (!result.has_value()) {
+          LOG(WARNING) << "e_governmentos: Toggle failed: " << result.error();
+        } else {
+          LOG(INFO) << "e_governmentos: Toggle result: " << result.value();
+        }
+      },
+      extension_id, bwi);
+}
