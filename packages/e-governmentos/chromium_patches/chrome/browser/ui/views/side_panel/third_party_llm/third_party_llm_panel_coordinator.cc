...
+
+// Preference names
+const char kThirdPartyLlmProvidersPref[] = "e_governmentos.third_party_llm.providers";
+const char kThirdPartyLlmSelectedProviderPref[] = "e_governmentos.third_party_llm.selected_provider";
+
...
+    LOG(INFO) << "[e_governmentos] Provider list size changed from " << previous_size
...
+  
+  e_governmentos_metrics::e-GovernmentOSMetrics::Log("llmchat.created");
+  
+  return container;
+}
+
...
+  if (!prefs) {
+    LOG(ERROR) << "[e_governmentos] Failed to get PrefService";
...
+      if (!item.is_dict()) {
+        LOG(WARNING) << "[e_governmentos] Invalid provider entry (not a dict), skipping";
+        continue;
+      }
+
...
+
+      if (!name || name->empty()) {
+        LOG(WARNING) << "[e_governmentos] Provider missing name, skipping";
+        continue;
+      }
+
+      if (!url || url->empty()) {
+        LOG(WARNING) << "[e_governmentos] Provider missing URL, skipping";
+        continue;
+      }
+
+      GURL provider_url(*url);
+      if (!provider_url.is_valid()) {
+        LOG(WARNING) << "[e_governmentos] Invalid provider URL: " << *url;
+        continue;
+      }
+
...
+  if (providers_.empty()) {
+    LOG(INFO) << "[e_governmentos] No providers in prefs, using defaults";
+    providers_ = GetDefaultProviders();
...
+    LOG(WARNING) << "[e_governmentos] Invalid selected provider index: " << selected_index
+                 << ", resetting to 0";
+    current_provider_index_ = 0;
+    prefs->SetInteger(kThirdPartyLlmSelectedProviderPref, 0);
+  } else {
...
+  PrefService* prefs = GetProfile()->GetPrefs();
+  if (!prefs) {
+    LOG(ERROR) << "[e_governmentos] Failed to get PrefService for saving";
+    return;
+  }
+
...
+    LOG(ERROR) << "[e_governmentos] Invalid provider index: " << new_provider_index;
+    return;
+  }
+
+  provider_change_in_progress_ = true;
+
+  e_governmentos_metrics::e-GovernmentOSMetrics::Log("llmchat.provider.changed");
+
+  if (owned_web_contents_) {
+    GURL current_url = owned_web_contents_->GetURL();
+    if (current_url.is_valid()) {
...
+  
+  e_governmentos_metrics::e-GovernmentOSMetrics::Log("llmchat.screenshot.captured");
+
...
+    
+    e_governmentos_metrics::e-GovernmentOSMetrics::Log("llmchat.content.copied");
+    
...
+  LOG(INFO) << "[e_governmentos] Media access requested for origin: "
...
+
+    LOG(INFO) << "[e_governmentos] Auto-granting media permission for trusted LLM: "
+              << origin_url.host();
+
...
+      VLOG(2) << "[e_governmentos] Media permission check for trusted LLM origin: "
...
+
+  VLOG(2) << "[e_governmentos] Media permission check for origin: "
+          << security_origin.Serialize()
+          << " type: " << static_cast<int>(type)
