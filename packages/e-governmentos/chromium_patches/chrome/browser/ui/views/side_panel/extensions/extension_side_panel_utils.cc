...
+  LOG(INFO) << "e_governmentos: IsContextualExtensionSidePanelOpen called for extension="
+            << extension_id;
+
+  if (!browser_window || !web_contents) {
+    LOG(WARNING) << "e_governmentos: browser_window or web_contents is null";
+    return false;
+  }
+
...
+  LOG(INFO) << "e_governmentos: is_active_tab=" << is_active_tab;
+
...
+    LOG(INFO) << "e_governmentos: side_panel is_showing=" << is_showing;
+    if (!is_showing) {
+      return false;
+    }
...
+    LOG(INFO) << "e_governmentos: contextual panel is_active=" << is_active;
+    return is_active;
+  }
+
...
+      tab->GetTabFeatures()->side_panel_registry();
+  bool is_active = IsKeyActiveInRegistry(contextual_registry, extension_key);
+  LOG(INFO) << "e_governmentos: inactive tab contextual panel is_active=" << is_active;
+  return is_active;
+}
+
...
+  LOG(INFO) << "e_governmentos: ToggleContextualExtensionSidePanel called for extension="
...
+  }
+
+  LOG(INFO) << "e_governmentos: is_currently_open=" << is_currently_open
...
+  }
+
+  LOG(INFO) << "e_governmentos: should_open=" << should_open;
+
+  // If already in desired state, return early.
+  if (should_open == is_currently_open) {
+    LOG(INFO) << "e_governmentos: Already in desired state, no action needed";
+    return is_currently_open;
+  }
+
+  if (!should_open) {
+    LOG(INFO) << "e_governmentos: Closing contextual panel";
+    side_panel_ui->Close();
+    contextual_registry->ResetActiveEntryFor(SidePanelEntry::PanelType::kContent);
+    return false;
+  } else {
+    LOG(INFO) << "e_governmentos: Opening contextual panel";
+
+    SidePanelEntry* contextual_entry =
+        contextual_registry->GetEntryForKey(extension_key);
+    LOG(INFO) << "e_governmentos: Got contextual_entry: "
+              << (contextual_entry ? "yes" : "no");
+
+    if (!contextual_entry) {
+      LOG(WARNING) << "e_governmentos: No contextual entry found, cannot open";
+      return false;
+    }
+
...
+
+    if (is_active_tab) {
+      LOG(INFO) << "e_governmentos: Calling side_panel_ui->Show() for active tab";
