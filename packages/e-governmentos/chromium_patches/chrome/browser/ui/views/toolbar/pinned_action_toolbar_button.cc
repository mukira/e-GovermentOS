...
+#include "chrome/browser/ui/actions/e_governmentos_actions_config.h"
...
+  // Set text from action item if available for e-GovernmentOS actions
+  if (auto* action_item = container_->GetActionItemFor(action_id)) {
+    if (e_governmentos::Ise-GovernmentOSAction(action_id)) {
+      // Check if labels should be shown
+      bool show_labels = true;
+      if (browser_ && browser_->profile()) {
+        show_labels = browser_->profile()->GetPrefs()->GetBoolean(
+            prefs::ke-GovernmentOSShowToolbarLabels);
+      }
+      else {
+      }
+      
...
+  // Update the text from the action item for e-GovernmentOS actions
+  if (e_governmentos::Ise-GovernmentOSAction(action_view_->GetActionId())) {
+    // Check if labels should be shown
+    bool show_labels = true;
