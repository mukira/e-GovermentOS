...
+  
+  // Don't show the dialog for e-GovernmentOS extensions
+  for (const char* allowed_id : extensions::e_governmentos::kAllowedExtensions) {
+    if (extension->id() == allowed_id) {
+      LOG(INFO) << "e_governmentos: Skipping settings override dialog for e-GovernmentOS extension " 
+                << extension->id();
+      return std::nullopt;
+    }
+  }
