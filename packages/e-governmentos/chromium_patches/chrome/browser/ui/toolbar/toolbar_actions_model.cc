...
+  // Check if it's a e-GovernmentOS extension
+  if (extensions::e_governmentos::Ise-GovernmentOSPinnedExtension(action_id)) {
+    return true;
+  }
+  
   auto* management =
...
+      
+  // Add e-GovernmentOS extensions to the force-pinned list (only those marked as pinned)
+  for (const std::string& ext_id : extensions::e_governmentos::Gete-GovernmentOSExtensionIds()) {
+    if (extensions::e_governmentos::Ise-GovernmentOSPinnedExtension(ext_id) &&
