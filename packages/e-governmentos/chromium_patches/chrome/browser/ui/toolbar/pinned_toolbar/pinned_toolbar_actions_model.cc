...
+  }
+
+  // Pin native e-GovernmentOS actions if their features are enabled (or no feature flag)
+  for (actions::ActionId id : e_governmentos::ke-GovernmentOSNativeActionIds) {
+    const base::Feature* feature = e_governmentos::GetFeatureFore-GovernmentOSAction(id);
