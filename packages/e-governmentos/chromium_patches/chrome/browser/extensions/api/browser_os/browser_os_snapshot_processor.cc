...
+            &is_offscreen);
+        
+        VLOG(3) << "[e_governmentos] Node " << node_data.id 
...
+    } else {
+      // No AXTree available
+      LOG(WARNING) << "[e_governmentos] No AXTree available for bounds computation";
+    }
+    
...
+  }
+  
+  LOG(INFO) << "[e_governmentos] Viewport: " << viewport_size.ToString() 
...
+  
+  if (!ax_tree) {
+    LOG(ERROR) << "[e_governmentos] Failed to create AXTree from update";
+    SnapshotProcessingResult result;
...
+  }
+  
+  LOG(INFO) << "[e_governmentos] Created AXTree with " << tree_update.nodes.size() 
