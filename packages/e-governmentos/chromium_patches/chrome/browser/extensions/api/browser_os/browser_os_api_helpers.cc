...
+  content::RenderFrameHost* rfh = web_contents->GetPrimaryMainFrame();
+  if (!rfh) {
+    LOG(WARNING) << "[e_governmentos] No RenderFrameHost for AccessibilityDoDefault";
+    return false;
+  }
+  
...
+  
+  LOG(INFO) << "[e_governmentos] Performing AccessibilityDoDefault on node " 
...
+  content::RenderFrameHost* rfh = web_contents->GetPrimaryMainFrame();
+  if (!rfh) {
+    LOG(WARNING) << "[e_governmentos] No RenderFrameHost for AccessibilityFocus";
+    return false;
+  }
+  
+  ui::AXActionData action_data;
...
+  action_data.target_tree_id = node_info.ax_tree_id;
+  
+  LOG(INFO) << "[e_governmentos] Performing AccessibilityFocus on node " 
+            << node_info.ax_node_id;
+  
+  rfh->AccessibilityPerformAction(action_data);
+  return true;
...
+  content::RenderFrameHost* rfh = web_contents->GetPrimaryMainFrame();
+  if (!rfh) {
+    LOG(WARNING) << "[e_governmentos] No RenderFrameHost for AccessibilityScrollToMakeVisible";
+    return false;
+  }
+  
+  ui::AXActionData action_data;
...
+  
+  LOG(INFO) << "[e_governmentos] Performing AccessibilityScrollToMakeVisible on node " 
+            << node_info.ax_node_id;
+  
+  rfh->AccessibilityPerformAction(action_data);
+  return true;
...
+    LOG(INFO) << "[e_governmentos] Node is out of viewport, scrolling to make visible";
...
+    // LOG(INFO) << "[e_governmentos] Node was out of viewport, trying AccessibilityDoDefault click first";
+    // bool changed = e-GovernmentOSChangeDetector::ExecuteWithDetection(
+    //     web_contents,
...
+      LOG(INFO) << "[e_governmentos] No change from accessibility click, trying HTML click";
+      changed = e-GovernmentOSChangeDetector::ExecuteWithDetection(
+          web_contents,
+          [&]() { HtmlClick(web_contents, node_info); },
+          base::Milliseconds(200));
+    }
+    
+    LOG(INFO) << "[e_governmentos] Click result: " << (changed ? "changed" : "no change");
+    return changed;
+  }
+  
+  // For in-viewport nodes, try coordinate click first (most natural)
+  LOG(INFO) << "[e_governmentos] Node is in viewport, trying coordinate click first";
+  gfx::PointF click_point = GetNodeCenterPoint(web_contents, node_info);
+  
+  bool changed = e-GovernmentOSChangeDetector::ExecuteWithDetection(
+      web_contents,
...
+  content::RenderFrameHost* rfh = web_contents->GetPrimaryMainFrame();
+  if (!rfh) {
+    LOG(WARNING) << "[e_governmentos] No RenderFrameHost for AccessibilitySetValue";
+    return false;
+  }
+  
+  ui::AXActionData action_data;
...
+  
+  LOG(INFO) << "[e_governmentos] Performing AccessibilitySetValue on node " 
...
+  
+  if (is_out_of_viewport) {
+    LOG(INFO) << "[e_governmentos] Node is out of viewport for typing, scrolling to make visible";
+    AccessibilityScrollToMakeVisible(web_contents, node_info, true /* center */);
+    // Wait for scroll to complete
+    base::PlatformThread::Sleep(base::Milliseconds(300));
+  }
...
+  if (!changed) {
+    LOG(INFO) << "[e_governmentos] No change from native typing, trying JavaScript";
+    changed = e-GovernmentOSChangeDetector::ExecuteWithDetection(
+        web_contents,
...
+  // if (!changed) {
+  //   LOG(INFO) << "[e_governmentos] No change from JavaScript, trying accessibility SetValue";
+  //   changed = e-GovernmentOSChangeDetector::ExecuteWithDetection(
+  //       web_contents,
+  //       [&]() {
...
+  // }
+  
+  LOG(INFO) << "[e_governmentos] Type result: " << (changed ? "changed" : "no change");
+  return changed;
+}
+
...
+      base::Milliseconds(200));
+  
+  LOG(INFO) << "[e_governmentos] Clear result: " << (changed ? "changed" : "no change");
+  return changed;
+}
+
...
+      base::Milliseconds(200));
+  
+  LOG(INFO) << "[e_governmentos] KeyPress result for '" << key << "': " 
...
+    LOG(INFO) << "[e_governmentos] No interactive elements in viewport to highlight";
+    return;
+  }
+  
+  LOG(INFO) << "[e_governmentos] Highlighting " << filtered_nodes.size() 
...
+      document.querySelectorAll('.e_governmentos-bbox').forEach(e => e.remove());
+      
+      // Create container for all bounding boxes
+      const container = document.createElement('div');
+      container.className = 'e_governmentos-bbox-container';
...
+        box.className = 'e_governmentos-bbox';
...
+      document.querySelectorAll('.e_governmentos-bbox-container').forEach(e => e.remove());
+      document.querySelectorAll('.e_governmentos-bbox').forEach(e => e.remove());
+      
+      // Remove all highlight containers  
+      document.querySelectorAll('.e_governmentos-highlight-container').forEach(e => e.remove());
+      document.querySelectorAll('.e_governmentos-highlight').forEach(e => e.remove());
+      
+      // Remove any style elements we added
+      document.querySelectorAll('#e_governmentos-highlight-styles').forEach(e => e.remove());
+      
+      return true;
+    })();
+  )";
...
+                                   const gfx::PointF& point) {
+  LOG(INFO) << "[e_governmentos] ClickCoordinatesWithDetection at (" 
+            << point.x() << ", " << point.y() << ")";
+  
...
+      base::Milliseconds(300));
+  
+  LOG(INFO) << "[e_governmentos] Click coordinates result: " 
+            << (changed ? "changed" : "no change");
+  return changed;
+}
+
...
+                                    const gfx::PointF& point,
+                                    const std::string& text) {
+  LOG(INFO) << "[e_governmentos] TypeAtCoordinatesWithDetection at (" 
...
+  if (!changed) {
+    LOG(INFO) << "[e_governmentos] No change from native typing at coordinates, trying JS injection";
+    
...
+  }
+  
+  LOG(INFO) << "[e_governmentos] Type at coordinates result: " 
