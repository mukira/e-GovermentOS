...
+uint32_t e-GovernmentOSGetInteractiveSnapshotFunction::next_snapshot_id_ = 1;
+
+// Constructor and destructor implementations
+e-GovernmentOSGetInteractiveSnapshotFunction::e-GovernmentOSGetInteractiveSnapshotFunction() = default;
+e-GovernmentOSGetInteractiveSnapshotFunction::~e-GovernmentOSGetInteractiveSnapshotFunction() = default;
+
+ExtensionFunction::ResponseAction e-GovernmentOSGetAccessibilityTreeFunction::Run() {
...
+      base::BindOnce(
+          &e-GovernmentOSGetAccessibilityTreeFunction::OnAccessibilityTreeReceived,
+          this),
...
+}
+
+void e-GovernmentOSGetAccessibilityTreeFunction::OnAccessibilityTreeReceived(
...
+}
+
+// Implementation of e-GovernmentOSGetInteractiveSnapshotFunction
+
+ExtensionFunction::ResponseAction e-GovernmentOSGetInteractiveSnapshotFunction::Run() {
...
+    LOG(WARNING) << "[e_governmentos] Frame not stable for AX snapshot - skipping";
...
+  web_contents->RequestAXTreeSnapshot(
+      base::BindOnce(
+          &e-GovernmentOSGetInteractiveSnapshotFunction::OnAccessibilityTreeReceived,
+          this),
+      ui::AXMode(ui::AXMode::kWebContents | ui::AXMode::kExtendedProperties |
+                 ui::AXMode::kInlineTextBoxes),
+      /* max_nodes= */ 0,  // No limit
...
+}
+
+void e-GovernmentOSGetInteractiveSnapshotFunction::OnAccessibilityTreeReceived(
+    ui::AXTreeUpdate& tree_update) {
+  // Double-check frame is still valid before processing
+  if (!web_contents_) {
+    LOG(WARNING) << "[e_governmentos] WebContents gone during AX snapshot callback";
+    browser_os::InteractiveSnapshot empty_snapshot;
+    empty_snapshot.snapshot_id = next_snapshot_id_++;
+    empty_snapshot.timestamp = base::Time::Now().InMillisecondsFSinceUnixEpoch();
+    empty_snapshot.processing_time_ms = 0;
...
+    LOG(WARNING) << "[e_governmentos] Frame became unstable during AX snapshot callback";
+    browser_os::InteractiveSnapshot empty_snapshot;
+    empty_snapshot.snapshot_id = next_snapshot_id_++;
+    empty_snapshot.timestamp = base::Time::Now().InMillisecondsFSinceUnixEpoch();
+    empty_snapshot.processing_time_ms = 0;
...
+      web_contents_,
+      base::BindOnce(
+          &e-GovernmentOSGetInteractiveSnapshotFunction::OnSnapshotProcessed,
+          base::WrapRefCounted(this)));
+}
+
+void e-GovernmentOSGetInteractiveSnapshotFunction::OnSnapshotProcessed(
+    SnapshotProcessingResult result) {
+  Respond(ArgumentList(
+      browser_os::GetInteractiveSnapshot::Results::Create(result.snapshot)));
+}
+
+// Implementation of e-GovernmentOSClickFunction
+
+ExtensionFunction::ResponseAction e-GovernmentOSClickFunction::Run() {
...
+}
+
+// Implementation of e-GovernmentOSInputTextFunction
+
+ExtensionFunction::ResponseAction e-GovernmentOSInputTextFunction::Run() {
...
+  const NodeInfo& node_info = node_it->second;
+  
+  LOG(INFO) << "[e_governmentos] InputText: Starting input for nodeId: " << params->node_id;
+  
...
+  
+  if (!change_detected) {
+    LOG(WARNING) << "[e_governmentos] InputText: No change detected after typing";
+  }
+  
+  // Create interaction response
+  browser_os::InteractionResponse response;
...
+}
+
+// Implementation of e-GovernmentOSClearFunction
+
+ExtensionFunction::ResponseAction e-GovernmentOSClearFunction::Run() {
...
+  const NodeInfo& node_info = node_it->second;
+  
+  LOG(INFO) << "[e_governmentos] Clear: Clearing field for nodeId: " << params->node_id;
+  
+  // Use ClearWithDetection which handles focus and clearing
+  bool change_detected = ClearWithDetection(web_contents, node_info);
+  
+  if (!change_detected) {
+    LOG(WARNING) << "[e_governmentos] Clear: No change detected after clearing";
+  }
+  
+  // Create interaction response
+  browser_os::InteractionResponse response;
...
+}
+
+// Implementation of e-GovernmentOSGetPageLoadStatusFunction
+
+ExtensionFunction::ResponseAction e-GovernmentOSGetPageLoadStatusFunction::Run() {
...
+}
+
+// Implementation of e-GovernmentOSScrollUpFunction
+
+ExtensionFunction::ResponseAction e-GovernmentOSScrollUpFunction::Run() {
...
+}
+
+// Implementation of e-GovernmentOSScrollDownFunction
+
+ExtensionFunction::ResponseAction e-GovernmentOSScrollDownFunction::Run() {
...
+}
+
+// Implementation of e-GovernmentOSScrollToNodeFunction
+
+ExtensionFunction::ResponseAction e-GovernmentOSScrollToNodeFunction::Run() {
...
+}
+
+// Implementation of e-GovernmentOSSendKeysFunction
+
+ExtensionFunction::ResponseAction e-GovernmentOSSendKeysFunction::Run() {
...
+  }
+  
+  LOG(INFO) << "[e_governmentos] SendKeys: Sending key '" << params->key << "'";
+  
+  // Send the key with change detection
+  bool change_detected = KeyPressWithDetection(web_contents, params->key);
+  
+  if (!change_detected) {
+    LOG(WARNING) << "[e_governmentos] SendKeys: No change detected after key press";
+  }
+  
+  // Create interaction response
+  browser_os::InteractionResponse response;
...
+}
+
+// Implementation of e-GovernmentOSCaptureScreenshotFunction
+
+e-GovernmentOSCaptureScreenshotFunction::e-GovernmentOSCaptureScreenshotFunction() = default;
+e-GovernmentOSCaptureScreenshotFunction::~e-GovernmentOSCaptureScreenshotFunction() = default;
+
+ExtensionFunction::ResponseAction e-GovernmentOSCaptureScreenshotFunction::Run() {
...
+    LOG(INFO) << "[e_governmentos] CaptureScreenshot: Using exact dimensions: "
...
+      LOG(INFO) << "[e_governmentos] CaptureScreenshot: Using thumbnail size: " << max_dimension 
...
+      LOG(INFO) << "[e_governmentos] CaptureScreenshot: Using viewport size: " << max_dimension;
+    }
+    
...
+}
+
+void e-GovernmentOSCaptureScreenshotFunction::DrawHighlightsAndCapture() {
...
+      LOG(INFO) << "[e_governmentos] Drawing highlights for screenshot with " 
+                << tab_it->second.size() << " interactive elements";
+      ShowHighlights(web_contents_, tab_it->second, true /* show_labels */);
+    } else {
+      LOG(INFO) << "[e_governmentos] No snapshot data available for highlighting";
+    }
+    
...
+        FROM_HERE,
+        base::BindOnce(&e-GovernmentOSCaptureScreenshotFunction::CaptureScreenshotNow,
+                       base::WrapRefCounted(this)),
...
+}
+
+void e-GovernmentOSCaptureScreenshotFunction::CaptureScreenshotNow() {
+  if (!web_contents_) {
...
+      target_size_,
+      base::BindOnce(&e-GovernmentOSCaptureScreenshotFunction::OnScreenshotCaptured,
+                     this));
+}
+
+void e-GovernmentOSCaptureScreenshotFunction::OnScreenshotCaptured(
...
+}
+
+// e-GovernmentOSGetSnapshotFunction implementation
+ExtensionFunction::ResponseAction e-GovernmentOSGetSnapshotFunction::Run() {
...
+  // Request accessibility tree snapshot
+  web_contents->RequestAXTreeSnapshot(
+      base::BindOnce(&e-GovernmentOSGetSnapshotFunction::OnAccessibilityTreeReceived,
+                     this),
+      ui::AXMode(ui::AXMode::kWebContents | ui::AXMode::kExtendedProperties),
+      /* max_nodes= */ 0,  // No limit
+      /* timeout= */ base::TimeDelta(),
...
+}
+
+void e-GovernmentOSGetSnapshotFunction::OnAccessibilityTreeReceived(
+    ui::AXTreeUpdate& tree_update) {
+  if (!has_callback()) {
+    return;
+  }
...
+}
+
+// e-GovernmentOSGetPrefFunction
+ExtensionFunction::ResponseAction e-GovernmentOSGetPrefFunction::Run() {
...
+}
+
+// e-GovernmentOSSetPrefFunction
+ExtensionFunction::ResponseAction e-GovernmentOSSetPrefFunction::Run() {
+  std::optional<browser_os::SetPref::Params> params =
+      browser_os::SetPref::Params::Create(args());
+  EXTENSION_FUNCTION_VALIDATE(params);
+
+  // Security: only allow modifying e_governmentos.* prefs
+  if (!params->name.starts_with("e_governmentos.")) {
+    return RespondNow(Error("Only e_governmentos.* preferences can be modified"));
+  }
+
+  Profile* profile = Profile::FromBrowserContext(browser_context());
+  PrefService* prefs = FindPrefService(params->name, profile);
...
+}
+
+// e-GovernmentOSGetAllPrefsFunction
+ExtensionFunction::ResponseAction e-GovernmentOSGetAllPrefsFunction::Run() {
+  Profile* profile = Profile::FromBrowserContext(browser_context());
+  PrefService* profile_prefs = profile->GetPrefs();
+  PrefService* local_state = g_browser_process->local_state();
+
+  // Build a combined e_governmentos prefs dict from both sources
+  base::Value::Dict combined_e_governmentos;
+
+  // Lambda to merge e_governmentos prefs from a PrefService
...
+
+    // Look for "e_governmentos" key in the top-level dict
+    const base::Value* e_governmentos_value = pref_dict.Find("e_governmentos");
+    if (e_governmentos_value && e_governmentos_value->is_dict()) {
+      // Merge this e_governmentos dict into combined
+      combined_e_governmentos.Merge(e_governmentos_value->GetDict().Clone());
+      LOG(INFO) << "[e_governmentos] GetAllPrefs: Found e_governmentos.* prefs in " << source_name;
+    }
+  };
+
...
+
+  // Create single PrefObject with the entire e_governmentos dict
+  std::vector<browser_os::PrefObject> pref_objects;
+  browser_os::PrefObject pref_obj;
+  pref_obj.key = "e_governmentos";
+  pref_obj.type = "dictionary";
+  pref_obj.value = base::Value(std::move(combined_e_governmentos));
...
+}
+
+// e-GovernmentOSLogMetricFunction
+ExtensionFunction::ResponseAction e-GovernmentOSLogMetricFunction::Run() {
...
+    
+    e_governmentos_metrics::e-GovernmentOSMetrics::Log(prefixed_event, std::move(properties));
+  } else {
+    // No properties, just log with extension ID
+    e_governmentos_metrics::e-GovernmentOSMetrics::Log(prefixed_event, {
...
+}
+
+// e-GovernmentOSGetVersionNumberFunction
+ExtensionFunction::ResponseAction e-GovernmentOSGetVersionNumberFunction::Run() {
...
+}
+
+// e-GovernmentOSExecuteJavaScriptFunction
+ExtensionFunction::ResponseAction e-GovernmentOSExecuteJavaScriptFunction::Run() {
...
+  }
+  
+  LOG(INFO) << "[e_governmentos] ExecuteJavaScript: Executing code in tab " << tab_info->tab_id;
+  
...
+      js_code,
+      base::BindOnce(&e-GovernmentOSExecuteJavaScriptFunction::OnJavaScriptExecuted,
+                     this),
...
+}
+
+void e-GovernmentOSExecuteJavaScriptFunction::OnJavaScriptExecuted(base::Value result) {
+  LOG(INFO) << "[e_governmentos] ExecuteJavaScript: Execution completed";
+
+  if (result.is_none()) {
...
+}
+
+// Implementation of e-GovernmentOSClickCoordinatesFunction
+ExtensionFunction::ResponseAction e-GovernmentOSClickCoordinatesFunction::Run() {
...
+                                       &error_message);
+  if (!tab_info) {
+    LOG(ERROR) << "[e_governmentos] ClickCoordinates: " << error_message;
+    browser_os::InteractionResponse response;
...
+  
+  LOG(INFO) << "[e_governmentos] ClickCoordinates: Clicking at (" 
+            << params->x << ", " << params->y << ")";
+  
...
+  
+  LOG(INFO) << "[e_governmentos] ClickCoordinates: Result = " 
...
+}
+
+// Implementation of e-GovernmentOSTypeAtCoordinatesFunction  
+ExtensionFunction::ResponseAction e-GovernmentOSTypeAtCoordinatesFunction::Run() {
...
+                                       &error_message);
+  if (!tab_info) {
+    LOG(ERROR) << "[e_governmentos] TypeAtCoordinates: " << error_message;
+    browser_os::InteractionResponse response;
+    response.success = false;
+    return RespondNow(ArgumentList(
...
+  gfx::PointF click_point(params->x, params->y);
+  
+  LOG(INFO) << "[e_governmentos] TypeAtCoordinates: Clicking at (" 
...
+  response.success = success;
+  
+  LOG(INFO) << "[e_governmentos] TypeAtCoordinates: Result = " 
