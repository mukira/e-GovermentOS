...
+    LOG(INFO) << "e_governmentos: ExtractPageContent - tree is empty";
+    return items;
+  }
+
+  LOG(INFO) << "e_governmentos: ExtractPageContent - processing " << tree_update.nodes.size() << " nodes";
+
...
+
+  LOG(INFO) << "e_governmentos: ExtractPageContent - extracted " << items.size() << " items";
+
+  return items;
+}
+
