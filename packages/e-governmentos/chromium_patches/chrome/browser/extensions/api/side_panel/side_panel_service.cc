...
+  LOG(INFO) << "e_governmentos: BrowserosToggleSidePanelForTab called for tab_id="
...
+      !window || !web_contents) {
+    LOG(WARNING) << "e_governmentos: Tab not found for tab_id=" << tab_id;
...
+  if (!browser_window) {
+    LOG(WARNING) << "e_governmentos: No browser window for tab_id=" << tab_id;
...
+  }
+
+  LOG(INFO) << "e_governmentos: has_contextual_options=" << has_contextual_options
+            << " for tab_id=" << tab_id;
+
...
+      LOG(WARNING) << "e_governmentos: No side panel path configured for extension="
+                   << extension.id();
+      return base::unexpected(
...
+    }
+
+    LOG(INFO) << "e_governmentos: Auto-registering contextual panel for tab_id="
...
+    LOG(WARNING) << "e_governmentos: Side panel is disabled for tab_id=" << tab_id;
+    return base::unexpected(
+        base::StringPrintf("Side panel is disabled for tabId: %d", tab_id));
+  }
+
+  // Toggle the contextual panel.
+  LOG(INFO) << "e_governmentos: Calling ToggleContextualExtensionSidePanel for tab_id="
+            << tab_id;
...
+
+  LOG(INFO) << "e_governmentos: Toggle result: is_now_open=" << is_now_open
+            << " for tab_id=" << tab_id;
+
+  return is_now_open;
+}
...
+    int tab_id,
+    bool include_incognito_information) {
+  LOG(INFO) << "e_governmentos: BrowserosIsSidePanelOpenForTab called for tab_id="
+            << tab_id << ", extension=" << extension.id();
+
+  // Find the tab.
+  WindowController* window = nullptr;
...
+  api::side_panel::PanelOptions current_options = GetOptions(extension, tab_id);
+  if (!current_options.enabled.value_or(true)) {
+    LOG(INFO) << "e_governmentos: Panel is disabled for tab_id=" << tab_id
+              << ", returning false";
+    return false;
+  }
+
...
+
+  LOG(INFO) << "e_governmentos: IsOpen result: is_open=" << is_open
+            << " for tab_id=" << tab_id;
+
+  return is_open;
+}
