...
+    // Check if this is a e-GovernmentOS extension
+    bool is_e_governmentos_extension = false;
+    for (const char* allowed_id : e_governmentos::kAllowedExtensions) {
+      if (extension->id() == allowed_id) {
+        is_e_governmentos_extension = true;
+        break;
+      }
+    }
+
+    if (!is_e_governmentos_extension) {
