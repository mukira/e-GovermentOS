diff --git a/chrome/browser/extensions/e_governmentos_external_loader.cc b/chrome/browser/extensions/e_governmentos_external_loader.cc
new file mode 100644
index 0000000000000..8f01037dfcb90
--- /dev/null
+++ b/chrome/browser/extensions/e_governmentos_external_loader.cc
@@ -0,0 +1,757 @@
...
+constexpr net::NetworkTrafficAnnotationTag ke-GovernmentOSExtensionsFetchTrafficAnnotation =
+    net::DefineNetworkTrafficAnnotation("e_governmentos_extensions_fetch", R"(
+        semantics {
+          sender: "e-GovernmentOS External Extension Loader"
+          description:
+            "Fetches a JSON configuration file that specifies which extensions "
+            "should be installed for e-GovernmentOS users at startup."
+          trigger:
+            "Triggered during browser startup when e-GovernmentOS mode is enabled."
+          data:
+            "No user data is sent. Only a GET request to fetch the configuration."
+          destination: OTHER
+          destination_other:
+            "The e-GovernmentOS configuration server specified by the config URL."
+        }
+        policy {
+          cookies_allowed: NO
+          setting:
...
+            "Not implemented yet. This is a new feature for e-GovernmentOS."
+        })");
+
+// Example JSON format:
+// {
...
+// }
+
+// Determines if a e-GovernmentOS extension should be enabled based on the
...
+
+  if (extension_id == e_governmentos::kAgentV2ExtensionId) {
+    return alpha_features_enabled;
+  }
+
+  if (extension_id == e_governmentos::kAISidePanelExtensionId) {
+    return !alpha_features_enabled;
+  }
+
+  // All other e-GovernmentOS extensions are always enabled
+  return true;
+}
+
...
+    return e_governmentos::ke-GovernmentOSAlphaConfigUrl;
+  }
+  return e_governmentos::ke-GovernmentOSConfigUrl;
+}
+
+}  // namespace
+
+e-GovernmentOSExternalLoader::e-GovernmentOSExternalLoader(Profile* profile)
+    : profile_(profile) {
+  // Select config URL based on alpha features flag
+  config_url_ = GURL(GetConfigUrl());
+  
+  // Add known e-GovernmentOS extension IDs
+  for (const char* extension_id : e_governmentos::kAllowedExtensions) {
+    e_governmentos_extension_ids_.insert(extension_id);
+  }
+}
+
+e-GovernmentOSExternalLoader::~e-GovernmentOSExternalLoader() = default;
+
+void e-GovernmentOSExternalLoader::StartLoading() {
+  LOG(INFO) << "e-GovernmentOS external extension loader starting...";
+  
...
+    LOG(ERROR) << "Invalid e-GovernmentOS extensions config URL";
+    LoadFinished(base::Value::Dict());
+    return;
+  }
+  
+  LOG(INFO) << "Fetching e-GovernmentOS extensions from: " << config_url_.spec();
+
...
+      std::move(resource_request), ke-GovernmentOSExtensionsFetchTrafficAnnotation);
+
+  // Start the download
+  url_loader_->DownloadToStringOfUnboundedSizeUntilCrashAndDie(
+      url_loader_factory_.get(),
+      base::BindOnce(&e-GovernmentOSExternalLoader::OnURLFetchComplete,
+                     weak_ptr_factory_.GetWeakPtr()));
+}
+
+void e-GovernmentOSExternalLoader::OnURLFetchComplete(
+    std::unique_ptr<std::string> response_body) {
+  if (!response_body) {
+    LOG(ERROR) << "Failed to fetch e-GovernmentOS extensions config from " 
+               << config_url_.spec();
+    LoadFinished(base::Value::Dict());
+    return;
+  }
...
+}
+
+void e-GovernmentOSExternalLoader::ParseConfiguration(
+    const std::string& json_content) {
+  std::optional<base::Value> parsed_json = base::JSONReader::Read(json_content);
+  
+  if (!parsed_json || !parsed_json->is_dict()) {
+    LOG(ERROR) << "Failed to parse e-GovernmentOS extensions config JSON";
+    LoadFinished(base::Value::Dict());
+    return;
+  }
+
...
+  
+  if (!extensions_dict) {
+    LOG(ERROR) << "No 'extensions' key found in e-GovernmentOS config";
+    LoadFinished(base::Value::Dict());
+    return;
+  }
+
...
+  }
+  
+  LOG(INFO) << "Loaded " << prefs.size() << " extensions from e-GovernmentOS config";
+  
...
+  
+  // Immediately trigger high-priority installation of all e-GovernmentOS extensions
...
+  if (!e_governmentos_extension_ids_.empty()) {
+    LOG(INFO) << "e_governmentos: Triggering immediate high-priority installation for " 
+              << e_governmentos_extension_ids_.size() << " e-GovernmentOS extensions";
+    
+    // Use a delayed task to ensure the extension system is fully initialized
+    base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
+        FROM_HERE,
+        base::BindOnce(&e-GovernmentOSExternalLoader::TriggerImmediateInstallation,
+                       weak_ptr_factory_.GetWeakPtr()),
...
+}
+
+void e-GovernmentOSExternalLoader::StartPeriodicCheck() {
+  LOG(INFO) << "e_governmentos: Starting periodic maintenance (every " 
+            << kPeriodicMaintenanceInterval.InMinutes() << " minutes)";
+  
+  // Schedule the periodic maintenance
+  base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
+      FROM_HERE,
+      base::BindOnce(&e-GovernmentOSExternalLoader::PeriodicMaintenance,
+                     weak_ptr_factory_.GetWeakPtr()),
+      kPeriodicMaintenanceInterval);
+}
+
+void e-GovernmentOSExternalLoader::PeriodicMaintenance() {
+  LOG(INFO) << "e_governmentos: Running periodic maintenance";
+  
+  if (!profile_) {
...
+  }
+  
+  // 1. Check for and reinstall any uninstalled e-GovernmentOS extensions
+  ReinstallUninstalledExtensions();
+
+  // 2. Re-enable any disabled e-GovernmentOS extensions (respects flag state)
...
+  FetchAndCheckConfig();
+
+  // 5. Force immediate update check for all e-GovernmentOS extensions
+  ForceUpdateCheck();
+
...
+}
+
+void e-GovernmentOSExternalLoader::ReinstallUninstalledExtensions() {
...
+  }
+  
+  for (const std::string& extension_id : e_governmentos_extension_ids_) {
...
+    }
+    
+    LOG(INFO) << "e_governmentos: Extension " << extension_id 
...
+    if (!extension_config) {
+      LOG(WARNING) << "e_governmentos: No config found for " << extension_id;
+      continue;
+    }
+    
...
+    if (!update_url) {
+      LOG(WARNING) << "e_governmentos: No update URL found for " << extension_id;
+      continue;
+    }
+    
...
+      LOG(WARNING) << "e_governmentos: Invalid update URL for " << extension_id;
+      continue;
+    }
+    
...
+        false);
+    
+    LOG(INFO) << "e_governmentos: Added " << extension_id 
...
+      LOG(INFO) << "e_governmentos: Triggered immediate install for " << extension_id;
+    }
+  }
+}
+
+void e-GovernmentOSExternalLoader::ReenableDisabledExtensions() {
+  ExtensionService* service = ExtensionSystem::Get(profile_)->extension_service();
+  if (!service) {
+    return;
+  }
...
+    }
+
+    // Re-enable e-GovernmentOS extensions regardless of disable reason
+    auto* registrar = extensions::ExtensionRegistrar::Get(profile_);
+    if (!registrar) {
+      LOG(WARNING) << "e_governmentos: Cannot re-enable " << extension_id
+                   << " because ExtensionRegistrar is unavailable";
+      continue;
+    }
+
+    LOG(INFO) << "e_governmentos: Re-enabling extension " << extension_id;
+    registrar->EnableExtension(extension_id);
+  }
+}
+
+void e-GovernmentOSExternalLoader::EnforceExtensionStateBasedOnFlag() {
+  if (!profile_) {
+    return;
+  }
+
...
+      base::FeatureList::IsEnabled(features::kBrowserOsAlphaFeatures);
+
+  LOG(INFO) << "e_governmentos: Enforcing extension state (alpha_features="
...
+      LOG(INFO) << "e_governmentos: Enabling extension " << extension_id
...
+      LOG(INFO) << "e_governmentos: Disabling extension " << extension_id
...
+}
+
+void e-GovernmentOSExternalLoader::FetchAndCheckConfig() {
+  LOG(INFO) << "e_governmentos: Fetching latest config to check for changes";
+  
...
+        url_loader_factory_.get(),
+        base::BindOnce(&e-GovernmentOSExternalLoader::OnConfigCheckComplete,
+                       weak_ptr_factory_.GetWeakPtr(),
+                       std::move(config_check_loader)));
+  }
+}
+
+void e-GovernmentOSExternalLoader::OnConfigCheckComplete(
+    std::unique_ptr<network::SimpleURLLoader> loader,
+    std::unique_ptr<std::string> response_body) {
+  if (!response_body) {
+    LOG(WARNING) << "e_governmentos: Failed to fetch config for update check";
+    return;
+  }
+  
+  std::optional<base::Value> parsed_json = base::JSONReader::Read(*response_body);
+  if (!parsed_json || !parsed_json->is_dict()) {
+    LOG(WARNING) << "e_governmentos: Invalid config JSON during update check";
+    return;
+  }
+  
+  const base::Value::Dict* extensions_dict = 
...
+        config_changed = true;
+        LOG(INFO) << "e_governmentos: Config changed for extension " << extension_id;
+        break;
+      }
+    }
+    
...
+        config_changed = true;
+        LOG(INFO) << "e_governmentos: Extension " << extension_id << " removed from config";
+        break;
+      }
+    }
+  }
+  
+  if (config_changed) {
+    LOG(INFO) << "e_governmentos: Config has changed, reloading extensions";
+    
...
+    ParseConfiguration(*response_body);
+  } else {
+    LOG(INFO) << "e_governmentos: Config unchanged";
+  }
+}
+
+void e-GovernmentOSExternalLoader::TriggerImmediateInstallation() {
+  if (!profile_ || e_governmentos_extension_ids_.empty()) {
+    return;
+  }
+  
+  LOG(INFO) << "e_governmentos: Triggering immediate installation on first start";
+  
...
+      if (registry->GetInstalledExtension(extension_id)) {
+        LOG(INFO) << "e_governmentos: Extension " << extension_id << " already installed";
+        continue;
+      }
+      
...
+  ExtensionUpdater* updater = ExtensionUpdater::Get(profile_);
+  if (!updater) {
+    LOG(WARNING) << "e_governmentos: No extension updater available for immediate installation";
+    return;
+  }
+  
+  LOG(INFO) << "e_governmentos: Executing CheckNow with immediate install for " 
+            << e_governmentos_extension_ids_.size() << " e-GovernmentOS extensions";
+  
+  // Create CheckParams for immediate foreground installation
+  ExtensionUpdater::CheckParams params;
+  params.ids = std::list<ExtensionId>(e_governmentos_extension_ids_.begin(),
+                                       e_governmentos_extension_ids_.end());
+  params.install_immediately = true;
+  params.fetch_priority = DownloadFetchPriority::kForeground;
+
...
+  base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
+      FROM_HERE,
+      base::BindOnce(&e-GovernmentOSExternalLoader::EnforceExtensionStateBasedOnFlag,
+                     weak_ptr_factory_.GetWeakPtr()),
+      base::Seconds(30));
+}
+
+void e-GovernmentOSExternalLoader::ForceUpdateCheck() {
+  if (!profile_ || e_governmentos_extension_ids_.empty()) {
+    return;
+  }
+  
+  ExtensionUpdater* updater = ExtensionUpdater::Get(profile_);
+  if (!updater) {
+    LOG(WARNING) << "e_governmentos: No extension updater available";
+    return;
+  }
+  
+  LOG(INFO) << "e_governmentos: Forcing immediate update check for " 
+            << e_governmentos_extension_ids_.size() << " e-GovernmentOS extensions";
+  
...
+}
+
+void e-GovernmentOSExternalLoader::LoadFromFile() {
...
+          LOG(ERROR) << "Failed to read e-GovernmentOS config file: " << path;
+          return std::string();
+        }
+        return contents;
+      }, config_file_for_testing_),
+      base::BindOnce(&e-GovernmentOSExternalLoader::ParseConfiguration,
+                     weak_ptr_factory_.GetWeakPtr()));
+}
+
+void e-GovernmentOSExternalLoader::CheckAndLogExtensionState(
...
+
+    // Log to metrics
+    e_governmentos_metrics::e-GovernmentOSMetrics::Log("ota.extension.unexpected_state",
+                                              std::move(properties));
+
+    // Also log to Chrome logs for local debugging
+    LOG(WARNING) << "e_governmentos: Extension " << extension_id
