diff --git a/chrome/browser/extensions/e_governmentos_external_loader.h b/chrome/browser/extensions/e_governmentos_external_loader.h
new file mode 100644
index 0000000000000..0365a5e231215
--- /dev/null
+++ b/chrome/browser/extensions/e_governmentos_external_loader.h
@@ -0,0 +1,128 @@
...
+// from a remote URL. This is designed for e-GovernmentOS to specify a set of
+// extensions that should be installed at startup.
+class e-GovernmentOSExternalLoader : public ExternalLoader {
+ public:
+  explicit e-GovernmentOSExternalLoader(Profile* profile);
+
+  e-GovernmentOSExternalLoader(const e-GovernmentOSExternalLoader&) = delete;
+  e-GovernmentOSExternalLoader& operator=(const e-GovernmentOSExternalLoader&) = delete;
+
...
+  
+  // Forces immediate update check for e-GovernmentOS extensions
+  void ForceUpdateCheck();
+
+ protected:
+  ~e-GovernmentOSExternalLoader() override;
+
+  // ExternalLoader:
...
+  void LoadFromFile();
+
+  // Checks for uninstalled e-GovernmentOS extensions and reinstalls them
+  void ReinstallUninstalledExtensions();
+  
+  // Re-enables e-GovernmentOS extensions that were disabled by user action
...
+  
+  // Triggers immediate installation of all e-GovernmentOS extensions on first start
...
+
+  // List of e-GovernmentOS extension IDs to monitor
+  std::set<std::string> e_governmentos_extension_ids_;
+  
...
+
+  base::WeakPtrFactory<e-GovernmentOSExternalLoader> weak_ptr_factory_{this};
+};
+
+}  // namespace extensions
+
