diff --git a/chrome/browser/resources/settings/e_governmentos_prefs_page/e_governmentos_prefs_page.ts b/chrome/browser/resources/settings/e_governmentos_prefs_page/e_governmentos_prefs_page.ts
new file mode 100644
index 0000000000000..463bbed53b5b3
--- /dev/null
+++ b/chrome/browser/resources/settings/e_governmentos_prefs_page/e_governmentos_prefs_page.ts
@@ -0,0 +1,249 @@
...
+
+/**
+ * @fileoverview 'settings-e_governmentos-prefs-page' contains e-GovernmentOS-specific settings.
+ */
+
...
+
+import {getTemplate} from './e_governmentos_prefs_page.html.js';
+
...
+}
+
+export interface Settingse-GovernmentOSPrefsPageElement {
+  $: {
+    addProviderDialog: HTMLElement;
+  };
+}
+
+const Settingse-GovernmentOSPrefsPageElementBase = PrefsMixin(PolymerElement);
+
+export class Settingse-GovernmentOSPrefsPageElement extends Settingse-GovernmentOSPrefsPageElementBase {
+  static get is() {
+    return 'settings-e_governmentos-prefs-page';
+  }
+
+  static get template() {
+    return getTemplate();
...
+    try {
+      const pref = this.getPref('e_governmentos.custom_providers');
+      if (pref && pref.value) {
...
+    this.setPrefValue('e_governmentos.custom_providers', customProvidersJson);
+  }
+  
+  /**
...
+    'settings-e_governmentos-prefs-page': Settingse-GovernmentOSPrefsPageElement;
+  }
+}
+
+customElements.define(
+    Settingse-GovernmentOSPrefsPageElement.is, Settingse-GovernmentOSPrefsPageElement);
