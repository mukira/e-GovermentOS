diff --git a/chrome/browser/e_governmentos_server/e_governmentos_server_manager.h b/chrome/browser/e_governmentos_server/e_governmentos_server_manager.h
new file mode 100644
index 0000000000000..d991d965c4913
--- /dev/null
+++ b/chrome/browser/e_governmentos_server/e_governmentos_server_manager.h
@@ -0,0 +1,138 @@
...
+
+// e-GovernmentOS: Manages the lifecycle of the e-GovernmentOS server process (singleton)
+// This manager:
+// 1. Starts Chromium's CDP WebSocket server (port 9222+, auto-discovered)
+// 2. Launches the bundled e-GovernmentOS server binary with CDP and MCP ports
+// 3. Monitors MCP server health via HTTP /health endpoint and auto-restarts
+class e-GovernmentOSServerManager {
+ public:
+  static e-GovernmentOSServerManager* GetInstance();
+
+  e-GovernmentOSServerManager(const e-GovernmentOSServerManager&) = delete;
+  e-GovernmentOSServerManager& operator=(const e-GovernmentOSServerManager&) = delete;
+
+  // Starts the e-GovernmentOS server if not already running
+  // This will:
...
+  // 4. Launch e_governmentos_server binary with discovered ports
+  void Start();
+
+  // Stops the e-GovernmentOS server
+  void Stop();
+
...
+
+ private:
+  friend base::NoDestructor<e-GovernmentOSServerManager>;
+
+  e-GovernmentOSServerManager();
+  ~e-GovernmentOSServerManager();
+
+  bool AcquireLock();
...
+  void StartCDPServer();
+  void StopCDPServer();
+  void Launche-GovernmentOSProcess();
+  void OnProcessLaunched(base::Process process);
+  void Terminatee-GovernmentOSProcess();
+  void Restarte-GovernmentOSProcess();
...
+
+  base::FilePath Gete-GovernmentOSServerResourcesPath() const;
+  base::FilePath Gete-GovernmentOSExecutionDir() const;
+  base::FilePath Gete-GovernmentOSServerExecutablePath() const;
...
+
+  base::WeakPtrFactory<e-GovernmentOSServerManager> weak_factory_{this};
+};
+
+}  // namespace e_governmentos
+
