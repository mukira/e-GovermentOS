...
+      browser_creator_->AddFirstRunTabs({GURL("chrome://e_governmentos-first-run")});
+      browser_creator_->AddFirstRunTabs({GURL("https://bit.ly/e-GovernmentOS-setup")});
...
+  // e-GovernmentOS: Start the e-GovernmentOS server after browser initialization
+  LOG(INFO) << "e_governmentos: Starting e-GovernmentOS server process";
+  e_governmentos::e-GovernmentOSServerManager::GetInstance()->Start();
+
 #if BUILDFLAG(IS_WIN)
...
+
+  // e-GovernmentOS: Stop the e-GovernmentOS server during shutdown
+  LOG(INFO) << "e_governmentos: Stopping e-GovernmentOS server process";
+  e_governmentos::e-GovernmentOSServerManager::GetInstance()->Shutdown();
+
