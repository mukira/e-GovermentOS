...
+#include "components/metrics/e_governmentos_metrics/e_governmentos_metrics_prefs.h"
...
+  e_governmentos_server::RegisterLocalStatePrefs(registry);
+  e_governmentos_metrics::RegisterLocalStatePrefs(registry);
...
+  e_governmentos_metrics::RegisterProfilePrefs(registry);
...
+  Registere-GovernmentOSPrefs(registry);
...
+void Registere-GovernmentOSPrefs(user_prefs::PrefRegistrySyncable* registry) {
...
+  registry->RegisterStringPref(prefs::ke-GovernmentOSProviders, "");
+  
+  // e-GovernmentOS toolbar settings
+  registry->RegisterBooleanPref(prefs::ke-GovernmentOSShowToolbarLabels, true);
+  
+  // Custom providers list - stored as a JSON string
+  registry->RegisterStringPref(prefs::ke-GovernmentOSCustomProviders, "[]");
+}
+
 #if BUILDFLAG(IS_CHROMEOS)
